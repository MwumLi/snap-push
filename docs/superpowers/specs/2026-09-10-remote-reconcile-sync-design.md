...
| 2 | ✓ | ✓ | ✗ | 置 `stale=true`，归入「远端已删」，用户可补齐/清理 |
...
- **网格（图库）**：展示当前目标下的全部图片——本地存在的 + 远端独有的（`remoteIndex` 中本地无同 md5 的项，带「远端独有」标识，缩略图走 `/api/remote-file`）。
  - 本机 → 本地原名；
...
- 删除确认框使用当前目标名称。
- 远端独有卡片只提供复制（远端路径）与删除（目标作用域）；拉回本地通过同步抽屉完成。
...
3. **卡片状态徽标**：图库卡片只在目标行展示两个状态——`远端独有`（远端有、本地无副本）与 `远端已删`（记录在、远端文件已被删）。传输方式徽标（本机/妙传/rsync/scp）不再出现在图库卡片上（仅上传结果行保留）。#3 恢复记录（本地有、远端有、缺记录）视为工具自我修复，按正常一致显示，无任何徽标；`time` 仍为恢复记录的时间。
4. **同步抽屉升级**：
